...
func (job *AwsDataRetrieveJob) FetchAwsUnusedServiceDecommissioning3DB(profile model.AwsProfile, seqValue uint64) error {
	...
	for _, w := range resultrdsSvc.DBInstances {
		...
		averageCPU := GetMetricStatistics(cfg, "DBInstanceIdentifier", *w.DBInstanceIdentifier, "CPUUtilization", "AWS/RDS", 3600, types.StatisticAverage, types.StandardUnitPercent, timePast, timeNow)
		countAverageCPU := 0
		...
		maxCPU := GetMetricStatistics(cfg, "DBInstanceIdentifier", *w.DBInstanceIdentifier, "CPUUtilization", "AWS/RDS", 3600, types.StatisticMaximum, types.StandardUnitPercent, timePast, timeNow)
		countMaxCPU := 0
